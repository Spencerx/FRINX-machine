function user_group_auth(request, permited_groups)
    ...
    local bearer_role="Bearer.ReadWrite"
    local service_role="L3VPN.ReadWrite,Site.ReadWrite"
    local network_role=""

    local bearer_node="bearer"
    local service_node="service"
    local network_node="network"
    ...
    if method ~= "GET" then
        ...
        custom_error("ROLE_PERMISSION_ERROR", 419)
    end
end
